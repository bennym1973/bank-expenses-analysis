...
if option_plot == 'יתרה הכנסות הוצאות':
    ...
    for bar in ax.patches:
        ...
        if height != 0:
            ax.text(bar.get_x() + bar.get_width() / 2, height, f'{height:,.0f}₪', ha='center', va='bottom' if height > 0 else 'top', fontsize=7, fontweight='bold')
    ...
    for bar in ax.containers[0]:
        height = bar.get_height()
        if height != 0:
            ax.text(bar.get_x() + bar.get_width() / 2, height + (abs(height) * 0.05), f'{height:,.0f}₪', ha='center', va='top', fontsize=6, fontweight='bold', color='black')
...
filtered_months = months_only if "הכל" in selected_months else selected_months
...
if selected_actions:
    ...
    if len(selected_actions) == 1:
        action_text = f"'{selected_actions[0]}'"
    else:
        action_text = f"{', '.join([f'\'{a}\'' for a in selected_actions])}"

    st.write(f"📜 פירוט עסקאות עבור {action_text} {month_text}:")
    st.dataframe(transaction_details_display)
else:
    st.warning("בחר לפחות פעולה אחת להצגת פירוט.")
